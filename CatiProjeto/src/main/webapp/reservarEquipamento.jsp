...
<body>
	...
	<script>
		...
		$('#verificarBtn').on('click', function () {
		    ...
		    // Validação de data final menor que a data inicial
		    if (new Date(dataFim) < new Date(dataInicio)) {
		        $('#resultado').html('<p class="text-danger">A data final não pode ser menor que a data inicial.</p>').show();
		        return;
		    }
		    ...
		    if (!idEstoque) {
		        $('#resultado').html('<p class="text-danger">ID do estoque não encontrado.</p>').show();
		        return;
		    }

		    // Faz a requisição AJAX se todas as validações forem passadas
		    $.ajax({
